...
exports.createProgramController = async (req, res) => {
  try {
    ...
    await auditAcademic(req, "CREATE", "Program", null, req.body);
  } catch (error) {
...
exports.updateProgramController = async (req, res) => {
  try {
    ...
    await auditAcademic(req, "UPDATE", "Program", req.params.id, req.body);
  } catch (error) {
    responseStatus(res, 400, "failed", error.message);
  }
};
...
exports.deleteProgramController = async (req, res) => {
  try {
    ...
    await auditAcademic(req, "DELETE", "Program", req.params.id);
    responseStatus(res, 200, "success", result);
  } catch (error) {
    responseStatus(res, 400, "failed", error.message);
  }
