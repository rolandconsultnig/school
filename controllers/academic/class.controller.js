...
exports.createClassLevelController = async (req, res) => {
  try {
    await createClassLevelService(req.body, req.userAuth.id, res, req.tenant);
    await auditAcademic(req, "CREATE", "ClassLevel", null, req.body);
  } catch (error) {
...
exports.updateClassLevelController = async (req, res) => {
  try {
    ...
    await auditAcademic(req, "UPDATE", "ClassLevel", req.params.id, req.body);
  } catch (error) {
    responseStatus(res, 400, "failed", error.message);
  }
};
...
exports.deleteClassLevelController = async (req, res) => {
  try {
    ...
    await auditAcademic(req, "DELETE", "ClassLevel", req.params.id);
    responseStatus(res, 200, "success", result);
  } catch (error) {
    responseStatus(res, 400, "failed", error.message);
  }
